...
void Update::on_getdata_clicked()
{
    ...
    if(qry.exec()){
        while(qry.next()){
            ...
            ui->lineEdit_cc->setText(qry.value(2).toString());
            ui->lineEdit_cgpa->setText(qry.value(3).toString());
...
void Update::on_pushButton_2_clicked()
{
    ...
    QSqlQuery qry;
    qry.prepare("update Students set Name='"+name+"',CompletedCredit='"+cc+"',CGPA='"+cgpa+"',Mobile='"+mobile+"',Address='"+address+"' where ID='"+id+"' ");
    if(qry.exec()){
        QMessageBox::information(this,"Updated...","information succesfully Updated...");
    }else{
        QMessageBox::critical(this,"error",qry.lastError().text());
    }
    log.closeconnection();
